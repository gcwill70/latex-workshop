...
\tikzstyle{io} = [rectangle, trapezium left angle=70, trapezium right angle=110, minimum width=cm, text centered, minimum width=2cm, minimum height=1cm, draw=black, fill=blue!30]
\tikzstyle{output} = [rectangle, rounded corners, minimum width=2cm, minimum height=1cm, text centered, draw=black, fill=green!30]
\tikzstyle{arrow} = [thick,->,>=stealth]

\begin{tikzpicture}[node distance=1cm]
	\node (pic) [input, yshift=1cm] {image file};
	\node (bib) [input, yshift=-1cm] {biblography.bib};
	\node (tex) [input, right of=pic, yshift=-1cm, xshift=2cm] {doc.tex};
	\node (rend) [io, right of=tex, xshift=2cm] {\LaTeX\ Renderer};
...
\begin{frame}[fragile]{Bibliography}
		...
		Some of the info in this presentation comes from \cite{oetiker_2003} and \cite{overleaf}.
...
\subsection*{Resume Demo}
\begin{frame}{Resume Demo}
	\begin{itemize}
		\item \LaTeX is great for resumes
		\item Easy to re-arrange and update content
		\item Easy to save revision history
		\item Easy to re-format when applying to different roles
		\item Let's create one using Overleaf!
		\item \href{https://www.overleaf.com/gallery/tagged/cv}{https://www.overleaf.com/gallery/tagged/cv}
	\end{itemize}	
\end{frame}
